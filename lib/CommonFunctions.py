...
translation_map_cache = {}

def loadTranslationsMap(plugin):  
    if plugin in translation_map_cache:
        return translation_map_cache[plugin]
    translation_map = {} 
    translation_map_cache[plugin] = translation_map
    xmlfiles = []
    pofiles = []
    for lang in ["English","en_us","en_gb","en_GB", "en_Gg","en_US","en_Us"]:
        xmlfiles.extend([
            "%s/%s/resources/language/%s/strings.xml" % (settings.PLUGINS_FOLDER, plugin, lang),
            "%s/%s/resources/language/resource.language.%s/strings.xml" % (settings.PLUGINS_FOLDER, plugin, lang)
        ])
        pofiles.extend([
            "%s/%s/resources/language/%s/strings.po" % (settings.PLUGINS_FOLDER, plugin, lang),
            "%s/%s/resources/language/resource.language.%s/strings.po" % (settings.PLUGINS_FOLDER, plugin, lang)
        ])        
    for xmlfile in xmlfiles:
        if os.path.exists(xmlfile):
            xmllocalization = ET.fromstring(open(xmlfile,"r").read())
            for string in xmllocalization.iter('string'):
                if "id" in string.attrib:
                    translation_map[int(string.attrib["id"])] = string.text
            return translation_map 
            
    for pofile in pofiles:
        if os.path.exists(pofile):
            content = open(pofile, "r").read()
            for part in content.split('msgctxt "#')[1:]:
                try: 
                    nr  = int(part.split('\n')[0].split('"')[0])
                    msg = part.split('msgid "')[1].split('"')[0]
                    translation_map[nr] = msg
                except Exception as e:  
                    pass
            return translation_map
            
    
    print( "loadTranslationsMap for plugin '%s' has no localized string file" %  xbmcWrapperCommon.CURRENT_PLUGIN )
    return translation_map
        
    
# This function raises a keyboard for user input
def getUserInput(title=u"Input", default=u"", hidden=False):
    print( "CommonFunctions.getUserInput(title = '%s', default = '', hidden = %s )" % (title,default,hidden))
    log("", 5)
    result = None
...
def getUserInputNumbers(title=u"Input", default=u""):
    print( "CommonFunctions.getUserInputNumbers(title = '%s', default = '')" % (title,default))
...
def getXBMCVersion():
    print( "CommonFunctions.getXBMCVersion()")
...
def getParameters(parameterString):
    print( "CommonFunctions.getParameters('parameterString')")
    parameterString1 = parameterString
    log(parameterString, 5)
    commands = {}
    ...
    for command in splitCommands:
        if (len(command) > 0):
            ...
            try: 
                ...
                raise Exception("fehler %s - %s - %s" % (command ,parameterString,parameterString1))
...
def log(description, level=0):
    ...
    try:
        print((u"[%s] %s : '%s'" % (plugin, inspect.stack()[1][3], description)).decode("utf-8"), xbmc.LOGNOTICE)
    except:
        print(u"FALLBACK [%s] %s : '%s'" % (plugin, inspect.stack()[1][3], repr(description)), xbmc.LOGNOTICE)
